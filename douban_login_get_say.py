...
class DoubanClient(object):
	...
	def login(self,username,password, #登陆的函数
		...
		for i in range(1,105):
			new_url = 'http://www.douban.com/people/jayhua/statuses?p='+str(i)

			res = self.session.get(new_url) 

			page = BeautifulSoup(res.text, 'html.parser')

			quote_list = page.find_all('blockquote')

			for quote in quote_list:
			    
			    quote_text = quote_text + quote.get_text() + '\r\n'

		with open('quote.text','w') as q:
			q.write(quote_text)
			q.close()
